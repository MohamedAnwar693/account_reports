...
class PartnerLedgerCustomHandler(models.AbstractModel):
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        # Utility function to update columns
        def _update_column(line, column_key, new_value, blank_if_zero=False):
            line['columns'][column_key]['name'] = self.env['account.report']._format_value(options, new_value,
                                                                                           figure_type='monetary',
                                                                                           blank_if_zero=blank_if_zero)
            line['columns'][column_key]['no_format'] = new_value

        # Utility function to update balance columns
        def _update_balance_columns(line, debit_column_key, credit_column_key, total_diff_values_key):
            ...
            credit_value = line['columns'][credit_column_key][
                'no_format'] if credit_column_key is not None else False

            if debit_value and credit_value:
                new_debit_value = 0.0
                new_credit_value = 0.0

                # Compare debit and credit values
        ...
        # Handling export_mode and filter_search_bar
        ...
        if not partner_lines:  # Ensure it's not None or empty before processing
            return []
        ...
        # Initialize total diff values for balances
        total_diff_values = {
            'initial_balance': 0.0,
            'end_balance': 0.0,
        }

        # Find the indices for debit and credit columns
        ...
        init_balance_credit_index = next(
            (index for index, column in enumerate(options['columns']) if
             column.get('expression_label') == 'credit'),
            None)

        end_balance_debit_index = -(next((index for index, column in enumerate(reversed(options['columns'])) if
                                          column.get('expression_label') == 'debit'), -1) + 1) or None
        end_balance_credit_index = -(next((index for index, column in enumerate(reversed(options['columns'])) if
                                           column.get('expression_label') == 'credit'), -1) + 1) or None

        for line in lines[:-1]:
            # Ensure proper access to the model info
            model_info = report._get_model_info_from_id(line['id'])

            # Handle the case when model_info is a tuple (first element contains the model)
            if isinstance(model_info, tuple):
                res_model = model_info[0]  # Access the first element of the tuple
            elif isinstance(model_info, dict):
                # Handle the case when model_info is a dictionary
                res_model = model_info.get('model')
            else:
                res_model = None  # Default to None if neither tuple nor dict

            # Now continue with your processing logic
            ...
            # Additional handling for model_info
            if isinstance(model_info, tuple):  # Avoid redundant call if it's already handled
                res_model = model_info[0]  # This line can now safely access the model

            if res_model == 'account.account':
                line['caret_options'] = 'partner_ledger'


        # Add total line adjustments
        if lines:
            total_line = lines[-1]
        ...
        # Return the updated lines as a list of tuples (0, line) format
        return [(0, line) for line in lines] if lines else []
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        """ Modifies the provided options to add custom conditions based on exchange rate journals. """
        ...
        if exch_code:
            # Construct domain to filter exchange-related entries
            domain += ['!', '&', '&', '&', ('credit', '=', 0.0), ('debit', '=', 0.0), ('amount_currency', '!=', 0.0),
                       ('journal_id', 'in', exch_code.ids)]

        # Apply the domain to options
        options['forced_domain'] = options.get('forced_domain', []) + domain

        # Handle multi-currency options
        if self.user_has_groups('base.group_multi_currency'):
            options['multi_currency'] = True

        # Assuming there's a need to manipulate columns like in the second example:
        # Initialize the options if necessary
        options.setdefault('comparison', {'periods': []})
        if options['comparison']['periods']:
            # Reverse the order of columns grouped by column_group_key
            new_columns_order = []
            current_column = []
        ...
        # Optionally, add new sections or group elements here similar to initial_balance or end_balance
        # For example, add custom balance options for the exchange rates:
        exchange_balance_options = self.env['account.general.ledger.report.handler']._get_options_initial_balance(
            options)
        exchange_forced_options = {
            'date': exchange_balance_options['date'],
            'include_current_year_in_unaff_earnings': exchange_balance_options['include_current_year_in_unaff_earnings']
        }
        exchange_balance_header_element = [{'name': _("Initial Balance"), 'forced_options': exchange_forced_options}]
        col_headers_exchange_balance = [
            exchange_balance_header_element,
            *options['column_headers'][1:],  # Insert after the initial column header
        ]
        exchange_column_group_vals = report._generate_columns_group_vals_recursively(col_headers_exchange_balance,
                                                                                     default_group_vals)
        exchange_columns, exchange_column_groups = report._build_columns_from_column_group_vals(exchange_forced_options,
                                                                                                exchange_column_group_vals)

        # Calculate end balance forced options
        end_date_to = options['date']['date_to']
        end_date_from = options['comparison']['periods'][-1]['date_from'] if options['comparison']['periods'] else \
        ...
        # End Balance Header Element
        end_balance_header_element = [{'name': _("End Balance"), 'forced_options': end_forced_options}]

        # Modify the column headers to include the End Balance section
        col_headers_end_balance = [
            end_balance_header_element,
            *options['column_headers'][1:],  # Insert after the initial column headers
        ]

        # Generate columns group values for the end balance
        end_balance_column_group_vals = report._generate_columns_group_vals_recursively(col_headers_end_balance,
                                                                                        default_group_vals)
        end_balance_columns, end_balance_column_groups = report._build_columns_from_column_group_vals(
            end_forced_options, end_balance_column_group_vals)

        # Update options with new columns and groups
        options['column_headers'][0] = exchange_balance_header_element + options['column_headers'][0] + end_balance_header_element
        options['column_groups'].update(exchange_column_groups)
        options['column_groups'].update(end_balance_column_groups)
        options['columns'] = exchange_columns + options['columns'] + end_balance_columns

        # Ensure options are ordered correctly
